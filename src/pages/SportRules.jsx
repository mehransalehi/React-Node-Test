...
import { sportRulesData } from '/src/assets/js/sport_rules_es.js';

const SportRules = () => {
    const [activeSport, setActiveSport] = useState("apuestas-en-partidos-de-fútbol");
    ...
    const filteredSports = useMemo(() => {
        if (!searchTerm) return sportRulesData;
        
        const term = searchTerm.toLowerCase();
        return sportRulesData.filter(sport =>
    ...
    const currentSport = useMemo(() => 
        sportRulesData.find(sport => sport.id === activeSport),
        [activeSport]
    );
    ...
    const renderMobileBackButton = (onClick, content) => (
        <button className="back-block">
            <span className="SVGInline back-block__arrow">
                <img className="SVGInline-svg back-block__arrow-svg" src={IconChevronLeft} alt="back" onClick={onClick} />
            </span>
    ...
    const renderContentSection = () => (
        <div className="sport-rules-layout__content">
            <h3 className="sport-rules-layout__content-title">
                {currentSport?.title || 'Reglas Deportivas'}
            </h3>
    ...
    return (
        <div className="sport-rules-layout sport-rules-layout_isSportPages">
            {isMobile && !isSelectRule && renderMobileBackButton(handleBack, "Reglas deportivas")}
            ...
            {isMobile && isSelectRule ? (
                <>
                    {renderMobileBackButton(() => setIsSelectRule(false), currentSport?.title || 'Reglas Deportivas')}
